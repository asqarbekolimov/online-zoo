...
async function getPets(): Promise<IPet[] | null> {
  showLoading()

  try {
    const { data } = await getData("pets")
    return data
  } catch (error) {
    console.error("Pets ma'lumotlarini olishda xatolik:", error)
    showError("Failed to load pets. Please try again.")
    return null
  }
}

function renderPets(pets: IPet[]) {
  if (!petsContainer) return
        ...
        <div class="card--navy">
          <div class="card_header">
            <div class="animal_name">
              <span class="subheader">${pet.name}</span>
            </div>
            <img class="card_img" src="/assets/images/card/${pet.id}.png" alt="${pet.name}" />
          </div>
          <div class="card_body">
            <h3>${pet.name}</h3>
            <p>${pet.description}</p>
            <a href="/pages/pet/${pet.id}">
              <button class="ghost-btn">
                <span class="btn-text">VIEW LIVE CAM</span>
                <svg
